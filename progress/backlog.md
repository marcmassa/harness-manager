...
- [ ] **Fix the e2e harness against current VS Code stable** — `@vscode/test-electron@2.5.2` spawns `Contents/MacOS/Electron` but darwin zips from 1.136.x ship the binary as `Code` (ENOENT before extension load; `npm run test:integration` fails identically on `main` and on `feat/react-flow-12`). The FEAT-037 e2e pass was obtained via `VSCODE_TEST_PATH` against cached 1.124.2. Fix = upgrade test-electron (or pin/patch the binary-name resolution) so the integration gate runs against stable again. Surfaced during FEAT-037 (2026-09-09), deliberately kept out of that feature (one feature at a time).
