...
- [x] **T7** — Implement `_runAgent()` in `RunCoordinator`: resolve adapter, build command,
...
- [x] **T14** — Add `'getRunAdapters'`, `'runAdapters'`, `'runAgent'`, `'agentRunStarted'`,
...
- [x] **T15** — Create `src/webview/RunAgentPanel.tsx` (~220 lines) with all controls
...
- [x] **T16** — Implement command preview auto-update in `RunAgentPanel`: recompute
...
- [ ] **T22** — Add "Runs" history button (clock icon) in the whiteboard toolbar;
  ...
  `setRunPanelNodeId(entry.nodeId)` and pre-fills the task _(R16)_ [deferred — Phase 2]
...
- [x] **T23** — Add `'createNodeFromWizard'` and `'generateAgentDescription'` and
  `'agentDescriptionResult'` to `WebviewMessageType` and `KNOWN_MESSAGE_TYPES` _(R17)_

- [x] **T24** — Create `src/webview/AgentBuilderWizard.tsx` (~300 lines): multi-step
...
- [x] **T25** — Implement Step 2 "✨ Generate with AI" in `AgentBuilderWizard`: post
...
- [x] **T30** — Create `src/whiteboard/architectureTemplates.ts` with the four built-in
...
- [x] **T31** — Add `'getArchitectureTemplates'`, `'architectureTemplates'`,
  `'applyArchitectureTemplate'` to message types _(R22)_

- [x] **T32** — Handle `getArchitectureTemplates` in `WhiteboardCoordinator`: return
  the template list (serialized, without file content) _(R22)_

- [x] **T33** — Handle `applyArchitectureTemplate` in `WhiteboardCoordinator`: loop
...
- [x] **T34** — Create `src/webview/ArchitectureTemplatePanel.tsx` (~150 lines):
...
- [x] **T35** — Add "Templates" (grid icon) button to whiteboard toolbar; clicking
  opens `<ArchitectureTemplatePanel>` as a slide-in _(R22)_

### Group G: Sync Utilities

- [x] **T36** — Register `harness-dashboard.scaffoldMissing` command in `extension.ts`
...
- [x] **T37** — Register `harness-dashboard.syncFromFilesystem` command: scans
...
- [x] **T38** — Add ⚙ menu to whiteboard toolbar with "Scaffold missing files" and
